...
// *쿼리 전송
// mysqli_query('연결객체','전송할쿼리');
$result = mysqli_query($dbcon, $sql);

// *DB에서 데이터 가져오기
// mysqli_fetch_row("전송한 쿼리"); // 컬럼 순서
// $row = mysqli_fetch_row($result);
// echo $row[1]; if($u_id == $row[0])

// mysqli_fetch_array("전송된 쿼리"); // 컬럼 이름
// $array = mysqli_fetch_array($result);
// echo $array['pwd']; if ($u_id == $array["u_id"])

// mysqli_num_rows("전송된 쿼리") // 전체 데이터 수
...
if(!$num){ // 일치하는 아이디가 없다면
  ...
  $array = mysqli_fetch_array($result); 
  $g_pwd = $array['pwd']; // DB에서 사용자 정보 가져오기
